...
当前项目由两个本地 AI 协同修改：**Codex 负责格局规划，Claude Code 负责任务执行**。两者都可以读写代码，但默认按下列边界协作，避免重复改动、方向漂移和上下文互相覆盖。

### 角色分工

| 角色 | 默认职责 | 不默认承担 |
|------|----------|------------|
| Codex | 架构判断、任务拆分、方案取舍、风险识别、验收标准、跨模块一致性、文档规则更新 | 大批量机械改代码、连续小步实现 |
| Claude Code | 按既定方案落地实现、补测试、跑命令验证、修编译错误、整理局部代码 | 未经确认的大方向重构、改技术路线、推翻既定边界 |

### 协作流程

1. **先定方向，再动代码**：涉及架构、模块边界、数据模型、AI 链路、存储方案、迁移策略时，先由 Codex 给出方案和验收标准，再交给 Claude Code 执行。
2. **小任务可直接执行**：明确的 Bug 修复、补测试、字段透传、样式微调、命令验证等，可由 Claude Code 直接改，但不得顺手做无关重构。
3. **执行结果要回填**：Claude Code 完成后应说明改了什么、验证了什么、遗留什么；Codex 后续根据结果决定是否调整规划或更新本文档。
4. **同一时间只认一个执行者**：同一批文件不要让两个 AI 同时改。若一个 AI 已开始改某个模块，另一个 AI 只做阅读、评审或规划，除非用户明确切换执行者。
5. **冲突以用户最新指令为准**：当 Codex 规划、Claude 实现、用户口头要求不一致时，优先级为：用户最新指令 > 本文档协作规则 > 既有实现习惯 > AI 自己的偏好。

### 交接格式

Codex 给 Claude Code 的任务应尽量包含：

- 背景：为什么要改，和当前项目目标的关系
- 范围：允许改哪些模块，哪些不要碰
- 方案：关键实现路径，不要求写到方法级细节
- 验收：至少包含编译/测试命令，必要时包含接口或手工验证步骤
- 风险：已知坑、兼容点、需要保留的行为

Claude Code 回传执行结果应尽量包含：

- 变更摘要：按业务影响说，不只列文件
- 验证结果：跑过哪些命令，失败原因是什么
- 偏离说明：如果实现和 Codex 方案不同，说明为什么
- 待确认点：需要用户或 Codex 再判断的地方

### 禁止事项

- 不因为“顺手”改命名、换框架、调目录、删旧逻辑。
- 不在没有验证的情况下宣称完成；无法验证要明确说明原因。
- 不把临时想法写进 `AGENTS.md` / `CLAUDE.md`，只有稳定约定、踩坑和阶段进度才写。
- 不让两个 AI 分别维护两套互相冲突的项目规则；`AGENTS.md` 与 `CLAUDE.md` 的核心规则应保持一致。
...
- [x] **Agent Phase C：任务验收层**（TaskLedger + TaskVerifier + ResponseComposer）
...
- [x] **Agent Phase D：记忆治理 + Prompt 精简 + 规划校验修复 + Executor 抽象**（2026-06-10：端到端接入完成）
