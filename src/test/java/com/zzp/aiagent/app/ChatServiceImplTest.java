...
class ChatServiceImplTest {
    ...
    void setUp() {
        ...
        when(workflowEngine.plan(any(ChatRequest.class), anyString()))
                .thenAnswer(invocation -> {
                    String chatId = invocation.getArgument(1);
                    String turnId = chatId + ":test-turn";
    ...
    void streamEmitsLifecycleAndTokens() {
        when(executor.stream(any())).thenReturn(Flux.just(
    ...
    void streamErrorReturnsStructuredFailure() {
        when(executor.stream(any())).thenReturn(Flux.error(new RuntimeException("network")));
